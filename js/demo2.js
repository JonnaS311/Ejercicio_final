...
function createLandscape(params){
  ...
  var contenedor = document.querySelector(".landscape")
  var ancho = window.innerWidth;
  var alto = window.innerHeight;

  var escena, renderer, camara;
  var terrain;
  ...
  function sceneSetup(){
    ...
    escena.background = fogColor;
    escena.fog = new THREE.Fog(fogColor, 0, 400);

    
    sky()

    camara = new THREE.PerspectiveCamera(60, ancho / alto, .1, 10000);
    camara.position.y = 8;
    camara.position.z = 4;
  ...
  function render(){
    ...
    terrain.material.uniforms.scroll.value = time + map(mouse.yDamped, 0, alto, 0, 4);
    terrain.material.uniforms.distortCenter.value = Math.sin(time) * 0.1;
    terrain.material.uniforms.roadWidth.value = map(mouse.xDamped, 0, ancho, 1, 4.5);

    camara.position.y = map(mouse.yDamped, 0, alto, 4, 11);
...
function titulosAnimados() {
  ...
  const glitch = (el,cycles) => {
    ...
    setTimeout(() => {
      TweenMax.set(el, {x: 0, y: 0, color: '#ddd4ff'});
      glitch(el, cycles-1);
